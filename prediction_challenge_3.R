...
train3$Favorite.song[train3$Favorite.song=="ACDC-ThunderStruck"] <- 1
train3$Favorite.song[train3$Favorite.song=="Camila_Cabello-Havana"] <- 2
train3$Favorite.song[train3$Favorite.song=="Drake-Gods_Plan"] <- 3
train3$Favorite.song[train3$Favorite.song=="Ed_Sheeran-Perfect"] <- 4
train3$Favorite.song[train3$Favorite.song=="Mozart-Die_Zauberflote"] <- 5
train3$Favorite.song[train3$Favorite.song=="Rage_Against_The_Machine-Testify"] <- 6
train3$Favorite.song[train3$Favorite.song=="Other"] <- 7
train3$Favorite.song <- as.numeric(train3$Favorite.song)

boxplot(train3$Favorite.song~train3$Party, las=1, main = "Favorite Songs of Party Members")

caloriesMean <- tapply(train3$Calories, train3$Party,mean)
caloriesMean
socialMean <- tapply(train3$Social.Media,train3$Party,mean)
socialMean
coffeeMean <- tapply(train3$Coffees, train3$Party,mean)
coffeeMean
songMean <- tapply(train3$Favorite.song, train3$Party, mean)
songMean

#Tree creation
tree <- rpart::rpart(Party ~ Coffees+Calories, data = train3)
pred.tree <- predict(tree, newdata = tillet,type = "class")
rpart.plot::rpart.plot(tree)
#View(pred.tree)

CrossValidation::cross_validate(train3,tree, 20, 0.8)

#error
#error <- mean(train3$Party!= pred.tree)
#error

submission3$Party <- pred.tree
write.csv(submission3, file="mysubmission3.csv", row.names=FALSE)