...
#Tree creation
tree <- rpart::rpart(Revenue ~. , data = train5, control = rpart.control(cp = 0.0000001))
pred.tree <- predict(tree, newdata = test5, type = "vector")
rpart.plot::rpart.plot(tree)
#View(pred.tree)

CrossValidation::cross_validate(train5,tree, 4, 0.8)

#mean((pred.tree-train5$Revenue)^2)

submission5$Revenue <- pred.tree
write.csv(submission5, file="mysubmission5.csv", row.names=FALSE)