...
majorMean <- tapply(train4$Earnings, train4$Major, mean)

boxplot(train4$Earnings~train4$Major)

#Tree creation
tree <- rpart::rpart(Earnings ~. , data = train4, control = rpart.control(cp = 0.0000001))
pred.tree <- predict(tree, newdata = students, type = "vector")
rpart.plot::rpart.plot(tree)
#View(pred.tree)

CrossValidation::cross_validate(train4,tree, 1, 0.8)

#error <- mean(train4$Earnings != pred.tree)
#error

perf.lm <- lm(Earnings ~ Number_Of_Professional_Connections+Major+GPA+Number_Of_Parking_Tickets, data= train4)
pred.tree <- predict(perf.lm, newdata= train4)
rpart.plot::rpart.plot(tree)
#mean((pred.tree-train4$Earnings)^2)

submission4$Earnings <- pred.tree
write.csv(submission4, file="mysubmission4.csv", row.names=FALSE)